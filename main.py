...
class MyApp(BaseApp):
    ...
    def remove_todo(self, widget):
        ...
        if error:
          self.add_error(error) #display an error message if a user tries to remove a to-do item that doesn't exist
    ...
    def remove_birthday(self, widget):
        ...
        if error:
          self.add_error(error) #an error message will display if the user incorrectly tries to remove a birthday not in the birthdays dictionary
